...
def categorize_style(checkin: untappd.Checkin) -> str:
    ...
    if "ipa" in beer_type:
        return "IPA"
    if (
        "pale ale" in beer_type
        or "bitter" in beer_type
        or "session" in beer_type
    ):
    ...
    if (
        "wheat" in beer_type
        or "hefeweizen" in beer_type
        or "witbier" in beer_type
    ):
        return "Wheat Beer"
    if (
...
def compute_markov_matrices(
    ...
    """Construct transition matrices and compute stationary vs marginal distributions."""
    styles_list = [s for s, _ in marginal_counts.most_common()]
    ...
    pi_marg = np.array(
        [marginal_counts[s] / total_marginal for s in styles_list]
    )

    return styles_list, col_labels, p_full, p_cont, pi_stat, pi_marg
...
def analyze_terminal_states(
    ...
    for s in styles_list:
        ...
        terminal_stats.append(
            (s, p_end, transition_counts[s]["Session End"], total)
        )
...
def plot_markov_style_transitions(
    ...
    ax1.set_xticklabels(col_labels, rotation=45, ha="right", fontsize=9)
    ax1.set_yticklabels(styles_list, rotation=0, fontsize=9)
    ...
    bars2 = ax2.barh(
        ...
        pi_stat * 100,
        bar_width,
        label="Stationary Distribution (Within-Session)",
        color="#DD8452",
        alpha=0.9,
    )

    ax2.set_yticks(y_pos)
    ax2.set_yticklabels(styles_list, fontsize=9)
    ax2.invert_yaxis()
...
def main(
    out_file: Optional[Union[Path, str]] = "scripts/out/markov_style_transitions.png",
) -> None:
    checkins = untappd.load_latest_checkins()
    sessions = segment_sessions(checkins)

    transition_counts, _, marginal_counts = extract_transitions(sessions)
    styles_list, col_labels, p_full, p_cont, pi_stat, pi_marg = (
    ...
    print("=" * 70)
    middle_after_heavy, triplet_counts, total_triplets = (
        analyze_palate_cleansers(sessions)
    )
    ...
    print("=" * 70)
    print(
        f"{'Style':22s} | {'Marginal':>9s} | {'Stationary':>11s} | {'Delta':>8s}"
    )
    print("-" * 58)
    for i, style in enumerate(styles_list):
        ...
        delta = p_s - p_m
        print(
            f"{style:22s} | {p_m:8.2%} | {p_s:10.2%} | {delta:+7.2%}"
        )
